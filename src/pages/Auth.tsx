...
const features = [
  {
    ...
    desc: 'All mechanics verified & background checked',
  },
  {
    icon: '📍',
  ...
  },
  {
    ...
    desc: 'Razorpay-powered, fully encrypted',
  },
  {
    icon: '📊',
    title: 'Live Tracking',
    desc: 'Track your service in real time',
  },
];
...
export const Login: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (loginThunk.fulfilled.match(result)) {
      toast.success(
        `Welcome back, ${result.payload.user.name.split(' ')[0]}!`
      );
  ...
  return (
    ...
    >
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        {error && (
          <div className="rounded-lg border border-red-300 bg-red-100 px-3 py-2 text-sm text-red-700 dark:border-red-800 dark:bg-red-950/40 dark:text-red-300">
            {error}
          </div>
        )}
...
export const Register: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (registerThunk.fulfilled.match(result)) {
      toast.success(
        'Account created! Welcome to AutoCare Hub 🎉'
      );
  ...
  return (
    <AuthLayout
      ...
      subtitle="Start booking services in minutes"
    >
      <form onSubmit={handleSubmit} className="space-y-4">
        <Input
          label="Full name"
...
const AuthLayout: React.FC<AuthLayoutProps> = ({
  ...
  <div className="min-h-screen flex">
    ...
    <div className="hidden lg:flex lg:w-5/12 xl:w-1/2 bg-gradient-to-br from-brand-500 to-brand-400 flex-col justify-between p-10 relative overflow-hidden">
      ...
      <div className="relative z-10">
        ...
        <p className="text-brand-100 text-sm mb-8">
          Join 50,000+ customers who trust us for honest,
          expert car care.
        </p>
      ...
      <p className="text-xs text-brand-200 relative z-10">
        © 2026 AutoCare Hub. All rights reserved.
      </p>
    </div>
    ...
    <div className="flex-1 flex items-center justify-center p-6 bg-gray-50 dark:bg-zinc-950">
      ...
      >
        <div className="mb-8">
          <h1 className="font-display font-bold text-2xl text-gray-900 dark:text-zinc-50 mb-1">
            {title}
          </h1>
