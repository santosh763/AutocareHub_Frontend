...
import { SERVICE_CATALOG, VEHICLE_TYPE_CONFIG } from "../types";
            ...
            <div className="p-3 space-y-2">
              {vLoad ? (
                ...
                vehicles?.map((v) => (
                  <div
                    key={v.id}
                    className="flex items-center gap-3 p-3 rounded-xl hover:bg-gray-50 dark:hover:bg-zinc-800 transition-colors cursor-pointer"
                    onClick={() => navigate("/vehicles")}
                  >
                    <div className="w-10 h-10 rounded-xl bg-brand-50 dark:bg-brand-400/10 flex items-center justify-center text-xl flex-shrink-0">
                      {VEHICLE_TYPE_CONFIG[v.vehicle_type]?.icon ?? "🚗"}
                    </div>
            ...
            <div className="divide-y divide-gray-100 dark:divide-zinc-800">
              {bLoad ? (
                ...
                recentBookings.map((booking) => (
                  <div key={booking.id}>
                    <div
                      className="flex items-center gap-4 px-5 py-3.5 hover:bg-gray-50 dark:hover:bg-zinc-800 transition-colors cursor-pointer"
                      onClick={() => navigate("/bookings")}
                    >
                      ...
                      <div className="text-right flex-shrink-0">
                        ...
                        {booking.status === 'pending' && booking.payment_status === 'pending' ? (
                          <button
                            ...
                            className="block w-full mt-1.5 px-2 py-1 text-[10px] font-bold bg-brand-400 text-white rounded-md hover:bg-brand-300 transition-colors"
                          >
                            PAY NOW
                          </button>
                        ) : (
                          <p className="text-xs font-semibold text-gray-700 dark:text-zinc-300 mt-1">
